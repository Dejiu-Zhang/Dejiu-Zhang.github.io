...
export function createBookReader({stopScroll,startScroll,reduced}){
 ...
 function open(id,card){
  ...
  front.className=`book-face book-front ${work.color}`;front.innerHTML=coverMarkup(work);back.className=`book-face book-back ${work.color}`;back.innerHTML=`<span>${work.type.toUpperCase()} / ${work.number}</span><strong>${work.title}</strong><span>Olivia Zhang<br>${work.status}</span>`;
